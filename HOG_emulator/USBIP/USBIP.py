        ...
        for field in self._fields_:
            if isinstance(field[1], BaseStucture):
                ...
                if 'si' == field[1]:
                    ...
                    values.append(getattr(self, field[0], 0).encode('utf-8'))
                else:
...
class USBDevice():
    ...
    async def handle_usb_request(self, usb_req):
        if usb_req.ep == 0:
            self.handle_usb_control(usb_req)
        else:
            await self.handle_data(usb_req)
...
class USBContainer:
    ...
    async def run(self, ip='0.0.0.0', port=3240):
        ...
        while 1:
            ...
            while 1:
                if not attached:
                    ...
                    await self.usb_devices[0].handle_usb_request(usb_req)
            conn.close()
